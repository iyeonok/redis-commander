...
module.exports = function() {
  ...
  function getConnections(req, res) {
    ...
    var distinctConnections = Array.from(new Set(req.app.locals.redisConnections.map(x=>x.options.host)))
    .map(host=> {
      return req.app.locals.redisConnections.find(x=>x.options.host == host);
    });

    res.json({
      'ok': true,
      'connections': distinctConnections.map(function(connection) {
        let retObj = {
